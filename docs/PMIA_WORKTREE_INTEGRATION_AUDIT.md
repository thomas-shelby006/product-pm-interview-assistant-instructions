...
| `.worktrees/pmia-final-integration` | `improvement/pmia-0.7.0` includes implementation checkpoint `4bcc2c2`; clean before this documentation checkpoint | Authoritative combined verification worktree. |
...
Exact `00e7bbb` passed 1,332/1,332 tests and all validators. The tenth browser run clicked the now-correct Resume control but showed the control-plane decision was not durable until Window 2 completed provider submission/proof. Commit `4bcc2c2` commits `active / resume_pending` before remote role work, finalizes `live` on success, and durably rolls back to protected Pause on failure. The widened matrix passed 196/196.
