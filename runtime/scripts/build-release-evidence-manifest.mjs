...
if (smoke.ok !== true || smoke.deterministicBrowser?.ok !== true) throw new Error('Smoke deterministic browser evidence did not pass');
const providerCanaryStatus = String(smoke.providerCanary?.status || 'skipped');
if (!['passed','limited','failed','skipped'].includes(providerCanaryStatus)) throw new Error('Smoke provider canary status is invalid');
if (smoke.adaptiveTurnScenariosOk !== true) throw new Error('Smoke Adaptive Turn scenarios are incomplete');
const requiredAdaptiveScenarios = ['authoritativeFinal','pauseResume','carryover','independentAccumulation','restartRecovery'];
for (const scenario of requiredAdaptiveScenarios) {
  if (smoke.adaptiveTurnScenarios?.[scenario]?.ok !== true) {
    throw new Error(`Smoke Adaptive Turn scenario ${scenario} is incomplete`);
  }
}
...
if (smoke.operationsCommandIsolation?.ok !== true) throw new Error('Smoke Operations Lab command-isolation evidence is incomplete');
for (const view of ['desktop','mobile','tiny','print']) {
  ...
  if (!operationsProof || operationsProof.horizontalOverflow === true || operationsProof.viewCount !== 10 || operationsProof.scenarioCount !== 5 || operationsProof.itemCount !== 4 || operationsProof.privacy !== 'safe' || operationsProof.localInteractionEvidence?.keyboardMoved !== true || operationsProof.localInteractionEvidence?.scenarioChanged !== true || operationsProof.localInteractionEvidence?.commandFreeSourceContract !== true) throw new Error(`Smoke Operations Lab ${view} evidence is incomplete`);
}
...
const value = canonical({
  format: 'pmia-release-evidence-v2',
  commit,
