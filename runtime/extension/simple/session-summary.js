import { markerCounts } from './markers.js';
...
function roleSummary(stages, role) {
  ...
  const attempts = rendered.length + failed.length;
  return {
    ...
    successRate:attempts ? rendered.length / attempts : null
  };
}

export function buildSessionSummary(snapshot = {}, markers = []) {
