...
export function deriveInboxTriageBoard(snapshot = {}, now = Date.now(), selectedView = 'urgent') {
  ...
  for(const item of ledger){ const id=String(item.id || ''); const meta=metadata(snapshot,id); const age=Math.max(0,now-Number(item.persistedAt || item.updatedAt || now)); const until=Math.max(0,Number(meta.deferUntil || 0)); const deferCondition=String(meta.deferCondition || 'none'); const deferred=deferCondition!=='none' && !(deferCondition==='until_time' && until>0 && until<=now); const row={ id, seq:Number(item.envelope?.seq || 0), state:status(item), priority:String(meta.priority || 'normal'), pinned:Boolean(meta.pinned), deferred, deferCondition, deferUntil:until, parentId:String(meta.parentId || ''), ageMs:age };
    if(['critical','high'].includes(row.priority) && !row.deferred && !['proven','archived'].includes(row.state)) views.urgent.push(row);
