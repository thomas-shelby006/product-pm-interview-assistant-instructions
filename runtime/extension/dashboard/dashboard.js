...
import { renderQuestionPath } from './render-question-path.js';
...
function analysisDuration(value) {
  return value === null || value === undefined || value === '' ? 'pending' : formatDuration(value);
}
...
function renderLiveAnswerAnalytics(snapshot) {
  ...
  const formatRole = value => value?.enabled
    ...
    : 'Not enabled';
  const timing = value => {
    if (!value?.enabled) return 'No comparison runtime registered.';
    if (value.firstTokenMs === null && value.totalResponseMs === null) {
      return `Timing pending · output ${value.outputWpm === null ? 'pending' : `${value.outputWpm} WPM`} · speak ${formatDuration(value.estimatedSpeakingMs || 0)}`;
    }
    return `First token ${value.firstTokenMs === null ? 'pending' : formatDuration(value.firstTokenMs)} · total ${value.totalResponseMs === null ? 'pending' : formatDuration(value.totalResponseMs)} · output ${value.outputWpm === null ? 'pending' : `${value.outputWpm} WPM`} · speak ${formatDuration(value.estimatedSpeakingMs || 0)}`;
  };
...
function renderOverview(snapshot, now) {
  ...
  renderQuestionPath({ document, snapshot });
...
function render(changedKeys = null) {
  ...
  renderCommandPalette();
  if (changed('ledger', 'ledgerCounts', 'batchState', 'mode', 'clock')) renderQueue(state.snapshot, now);
...
const toggleLiveDetails = byId('toggleLiveDetails');
if (toggleLiveDetails) {
  toggleLiveDetails.addEventListener('click', () => {
    const expanded = document.body.dataset.liveDetails !== 'true';
    document.body.dataset.liveDetails = String(expanded);
    toggleLiveDetails.setAttribute('aria-pressed', String(expanded));
    toggleLiveDetails.textContent = expanded ? 'Hide details' : 'Show details';
  });
}
...
setInterval(() => {
  if (state.snapshot) scheduleRender(['clock']);
}, 1000);
