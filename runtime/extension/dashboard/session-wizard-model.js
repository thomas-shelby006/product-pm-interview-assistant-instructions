export function deriveSessionWizard(snapshot = {}, stage = 'start') {
  if(stage==='end'){
    ...
    const steps=[
      ...
      { id:'export', label:'Export session evidence', complete:Boolean(snapshot.latestExportAt), command:'export_session' },
      { id:'privacy', label:'Verify privacy-safe support state', complete:snapshot.production?.diagnostics?.privacy?.safe === true, command:'export_support_bundle' },
