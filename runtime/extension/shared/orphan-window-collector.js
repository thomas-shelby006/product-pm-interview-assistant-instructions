export function collectOrphanManagedWindows({ sessions = [], windows = [], now = Date.now(), staleAfterMs = 120000 } = {}) {
  const owned = new Set();
  for (const session of Array.isArray(sessions) ? sessions : []) for (const role of ['sender','receiver','comparison','pilot']) {
  ...
  for (const window of Array.isArray(windows) ? windows : []) {
    ...
    if (!/^PMIA_(SENDER|RECEIVER|COMPARISON|DASHBOARD)_/.test(title)) continue;
