const VIEW_BY_CODE = Object.freeze({
  answer_no_response:['queue','receiverPolicyState'], receiver_draft_conflict:['queue','draftConflictState'],
...
export function deriveContextualNavigation(snapshot = {}) {
  ...
  const actionMode=decision?.actionMode || (decision?.command ? 'execute' : 'inspect');
  return { route, actionMode, command:actionMode === 'execute' ? decision?.command || '' : '', payload:actionMode === 'execute' ? decision?.payload || {} : {}, choices:actionMode === 'choose' ? [...(decision?.choices || [])] : [], title:decision?.title || 'No action required', detail:decision?.detail || 'The live system is caught up.' };
}