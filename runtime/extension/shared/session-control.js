function registeredPair(registry, sessionId) {
  ...
  const comparisonTabId = session?.comparison?.tabId;
  if (!Number.isInteger(senderTabId) || !Number.isInteger(receiverTabId) || senderTabId === receiverTabId) {
    return null;
  }
  return { sessionId: normalized, senderTabId, receiverTabId, comparisonTabId: Number.isInteger(comparisonTabId) ? comparisonTabId : null };
}
...
export async function exportManagedSession({ registry, sessionId, sendToTab }) {
  ...
  const tabIds = [pair.senderTabId, pair.receiverTabId, pair.comparisonTabId].filter((tabId, index, list) => Number.isInteger(tabId) && list.indexOf(tabId) === index);
