...
export function deriveProviderBatchBudget({
  ...
  if (success >= 4096) maxChars = Math.min(maxChars, Math.max(2048, Math.floor(success / Math.max(.5, Math.min(.95, Number(safetyRatio) || .85)))));
  ...
  return {
    ...
    source: failure ? 'recent_failure' : success >= 4096 ? 'recent_success' : capabilityComplete ? 'provider_default' : 'capability_degraded'
  };
}