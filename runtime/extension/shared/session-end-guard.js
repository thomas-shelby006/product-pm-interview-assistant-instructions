...
function latestOutboxCount(snapshot) {
  ...
  for (let index = timeline.length - 1; index >= 0; index -= 1) {
    if (timeline[index]?.type === 'outbox_state') {
      return Math.max(0, Number(timeline[index]?.data?.count || 0));
    }
  }
  return 0;
}

function pendingCommandCount(snapshot = {}) {
  const direct = Number(snapshot.dashboardOperations?.pending ?? snapshot.operationActivity?.pending);
  if (Number.isFinite(direct)) return Math.max(0, direct);
  return (Array.isArray(snapshot.commandJournal) ? snapshot.commandJournal : [])
    .filter(item => ['pending', 'responding'].includes(String(item?.state || ''))).length;
}
...
export function sessionEndCounts(snapshot = {}) {
  ...
  const adaptiveTurns = adaptiveSafety.actionable
    ? Math.max(1, adaptiveSafety.heldCount, adaptiveSafety.interruptionMemberIds.length)
    : 0;
  return {
    actionable: Math.max(ledgerActionable, summaryActionable),
    inFlight: Math.max(ledgerInFlight, summaryInFlight, activeMembers),
    unpersisted: latestOutboxCount(snapshot),
    pendingCommands: pendingCommandCount(snapshot),
    liveActive,
    unresolvedChoice,
    adaptiveTurns,
    phase
...
export function prepareSessionEnd(snapshot, {
  ...
  const canEnd = counts.actionable === 0
    && counts.inFlight === 0
    && counts.unpersisted === 0
    && counts.pendingCommands === 0
    && counts.liveActive === 0
    && counts.unresolvedChoice === 0
    && counts.adaptiveTurns === 0;
  return {
    token: String(token),
...
export function validateSessionEnd(prepared, { token, mode, now = Date.now(), currentCounts = null } = {}) {
  ...
  const countKeys = ['actionable', 'inFlight', 'unpersisted', 'pendingCommands', 'liveActive', 'unresolvedChoice', 'adaptiveTurns'];
  const changed = currentCounts && countKeys.some(key => Number(currentCounts[key] || 0) !== Number(prepared.counts?.[key] || 0));
  ...
  if (Number(counts.adaptiveTurns || 0) > 0 && normalizedMode !== 'archive_and_end') {
    return { ok: false, error: 'adaptive_turns_actionable', counts };
  }
