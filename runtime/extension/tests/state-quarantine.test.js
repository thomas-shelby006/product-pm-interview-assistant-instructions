...
test('runtime store quarantines a future schema without overwriting it', async () => {
  const key = 'pilot';
  const area = memoryArea({ [key]: { schemaVersion: 4, writerVersion: 'future', sessions: [{ sessionId: 'first' }] } });
  ...
  area.data[key] = { schemaVersion: 4, writerVersion: 'future', sessions: [{ sessionId: 'second' }] };
  store.resetCache();
  await assert.rejects(store.load(), /runtime_state_blocked:future_schema/);
