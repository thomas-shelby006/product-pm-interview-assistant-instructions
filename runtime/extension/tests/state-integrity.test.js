...
test('runtime envelope digest is independent of object key order', () => {
  const first = { schemaVersion: 2, writerVersion: '0.7.0', committedAt: 10, sessions: [{ sessionId: 's1', mode: 'paused' }] };
  const second = { sessions: [{ mode: 'paused', sessionId: 's1' }], committedAt: 10, writerVersion: '0.7.0', schemaVersion: 2 };
...
test('sealed runtime envelope detects content mutation', () => {
  const sealed = sealRuntimeEnvelope({ schemaVersion: 2, writerVersion: '0.7.0', committedAt: 10, sessions: [{ sessionId: 's1' }] });
...
test('store recovers a valid prior envelope after current digest mismatch', async () => {
  const key = 'pilot';
  const previous = sealRuntimeEnvelope({ schemaVersion: 2, writerVersion: '0.7.0', committedAt: 10, sessions: [{ sessionId: 'old', mode: 'paused' }] });
...
test('store blocks and quarantines when current and previous digests are invalid', async () => {
  const key = 'pilot';
  const invalid = { schemaVersion: 2, writerVersion: '0.7.0', committedAt: 10, integrityDigest: 'bad', sessions: [{ sessionId: 's1' }] };
