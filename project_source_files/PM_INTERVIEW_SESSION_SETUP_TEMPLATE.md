...
The system has four layers, aligned with the current 0.11.0 runtime and `docs/PMIA_CURRENT_SYSTEM_TECHNICAL_GUIDE.html`:
...
2. **Session context** — per-interview Resume + JD + structured metadata. Re-weights emphasis; never adds new facts or claims.
3. **Live transcript state** — authoritative finals protected by the service worker and the receiver's active/next batch ownership.
...
2. User presses `Alt+R` to open Session Studio.
3. Session Studio collects Resume, Job Description, structured metadata, and optional notes in memory.
4. Start/Launch opens the managed sender, receiver, and Runtime Pilot surfaces in Microsoft Edge Stable.
5. The extension content runtimes reach BOOT → REGISTERED → READY and the launcher arms the receiver with the in-memory boot/session context.
6. The Manifest V3 service worker owns authoritative interviewer-final sequencing and forwards protected batches from sender to receiver.
7. The receiver submits a frozen batch and acknowledges it only after matching rendered user-turn proof; Resume/JD are not used as transport storage.
...
These fields are emitted in the boot prompt under a `Session context:` block using the exact labels above. Only allow-listed safe metadata is eligible for review telemetry; Resume, JD, avoid text, and freeform notes remain in the AutoHotkey process rather than being retained as raw session-log text. Metadata only re-weights emphasis and depth; it never authorizes a new claim.

Today these values are entered through Session Studio's structured controls, with optional Additional notes available separately. The controls are part of the active launcher and are covered by the release validator.

Note on enforcement: `Avoid mentioning` and `Answer mode` are **prompt-level behaviors** — the assistant follows them via instructions in the boot prompt and Project files. The runtime does **not** apply a deterministic redaction filter for generated answers or a hard runtime word cap. Treat them as strong guidance, not guarantees.
